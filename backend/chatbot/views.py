...
def chat(request):
    ...
    try:
        ...
        system_prompt = (
            ...
            "\n\n"
            "⚠️ **중요 지침 (개인정보 동의 여부 확인)**:\n"
            "1. 만약 사용자의 정보(나이, 직업 등)가 '정보제공 미동의' 또는 '알 수 없음'으로 되어 있다면, "
            "답변 서두에 반드시 다음 내용을 정중하게 포함해줘:\n"
            "   '📢 고객님은 마이데이터 제공에 동의하지 않으셔서 정밀한 맞춤 추천은 어렵습니다. "
            "대신, 일반적인 인기 상품과 금융 정보를 바탕으로 안내해 드릴게요.'\n"
            "2. 이후에는 특정 개인의 상황을 가정하지 말고, 보편적으로 좋은 상품이나 "
            "검색된 [금융 상품 정보] 중에서 혜택이 좋은 것을 객관적으로 소개해줘.\n"
            "\n"
            "만약 정보가 정상적으로 제공되었다면, 해당 사용자의 성향에 맞춰서 친절하고 전문적으로 답변해줘. "
